...
import { User3, UserUpdate } from "../models/User";
...
return this.http.get(`${this.baseUrl}/courses/${id}/image`, { responseType: 'blob' })
    ...
    getUserById():Observable<User3>{
      console.log(`${this.baseUrl2}/user/${this.authService.loggedUser.id}`);
     return  this.http.get<User3>(`${this.baseUrl2}/user/${this.authService.loggedUser.id}`)
   
    }
    updateUser(data:FormData):Observable<UserUpdate>{
      console.log(`${this.baseUrl2}/user/${this.authService.loggedUser.id}`);
     return  this.http.put<UserUpdate>(`${this.baseUrl2}/user/${this.authService.loggedUser.id}`,data)
   
    }

    fetchImage2(): Observable<Blob>{
      return this.http.get(`${this.baseUrl2}/user/${this.authService.loggedUser.id}/image`, { responseType: 'blob' })
          }
